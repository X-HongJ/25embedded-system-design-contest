...
//    }
    @Bean
    public Queue queueBootloaderStatus(){
        LOGGER.info("bootloader状态队列："+ queueManager.getBootloaderStatusQueue().name);
        return new Queue(queueManager.getBootloaderStatusQueue().name);
    }
    @Bean
    public DirectExchange bootloaderExchange(){
        LOGGER.info("声明bootloader交换机：{}",queueManager.getBootloaderStatusQueue().exchange);
        return (DirectExchange)ExchangeBuilder.directExchange(queueManager.getBootloaderStatusQueue().exchange).durable(true).build();
    }

    @Bean
    public Binding bootloaderStatusQueueBinding(DirectExchange bootloaderExchange, Queue queueBootloaderStatus) {
        LOGGER.info("bind queueTTLThirdParty：{}", queueBootloaderStatus);
        return BindingBuilder
                .bind(queueBootloaderStatus)
                .to(bootloaderExchange)
                .with(queueManager.getBootloaderStatusQueue().routeKey);
    }
