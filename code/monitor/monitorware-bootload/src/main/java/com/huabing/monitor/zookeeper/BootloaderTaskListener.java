...
public class BootloaderTaskListener {
    ...
    public void preFetch() {
        ...
        if (listTasks.isEmpty()) return;
        for (String path : listTasks) {
            LOGGER.info("pre fetched tasks:" + path);
            BootloaderRequest task =
                    zookeeperManager.getTaskFromZookeeper(bootloaderTaskPath + "/" + path);
                        ...
                        new TreeCacheListener() {
                            ...
                            public void childEvent(
                                ...
                                switch (eventType) {
                                        //                    case NODE_ADDED:
                                        //
                                        // LOGGER.info("节点添加，{}",treeCacheEvent.getData().getPath());
                                        //                        break;
                                    case NODE_UPDATED:
                                        LOGGER.info("节点更新");
                                        BootloaderRequest request =
                                                zookeeperManager.getTaskFromZookeeper(
