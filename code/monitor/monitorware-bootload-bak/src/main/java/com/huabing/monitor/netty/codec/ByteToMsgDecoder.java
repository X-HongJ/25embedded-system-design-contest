...
public class ByteToMsgDecoder extends ByteToMessageDecoder {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(ByteToMsgDecoder.class);

    private final static short SHOOKHANDS = ByteUtils.getShort(new byte[]{(byte) 0xAA, (byte) 0xDD});
    // 板子被动更新起始包
    private final static short BOOTLOADSHO0KHANDS = ByteUtils.getShort(new byte[]{(byte) 0xBC, (byte) 0xDD});
    private final static short BOOTLOADSTATUS = ByteUtils.getShort(new byte[]{(byte) 0xCC, (byte) 0xDD});
    // 空板子初始化起始包
    private final static short BOOTLOADINIT = ByteUtils.getShort(new byte[]{(byte) 0xBB, (byte) 0xDD});
    private final static short UPDATE = ByteUtils.getShort(new byte[]{(byte) 0x99, (byte) 0xDD});
    ...
    private BaseMsg createBootloadStatus(byte length, byte[] info) {
        ...
        if (length <= 8) {
            ...
            bootloadStatus = new BootloadStatus(BOOTLOADSTATUS, length, stationId, boardId, status, end);
        }else {
            int deviceId = ByteUtils.getInt(new byte[]{info[0], info[1],info[2], info[3]});
            byte status = info[4];
            short end = ByteUtils.getShort(new byte[]{info[5], info[6]});
            bootloadStatus = new BootloadStatus(BOOTLOADSTATUS,length,(byte)0,(byte)0,status,end,deviceId);
        }
    ...
    private BaseMsg createBootloadShookHands(byte length, byte[] info) {
        ...
        if (length <= 7) {
            ...
            byte boardId = info[1];
            short end = ByteUtils.getShort(new byte[]{info[2], info[3]});
            bootloadShookhand =  new BootloadShookhand(BOOTLOADSHO0KHANDS, length, stationId, boardId, end);
        }else {
            int deviceId = ByteUtils.getInt(new byte[]{info[0], info[1],info[2], info[3]});
            short end = ByteUtils.getShort(new byte[]{info[4], info[5]});
            bootloadShookhand = new BootloadShookhand(BOOTLOADSHO0KHANDS, length, (byte) 0, (byte) 0, end, deviceId);
        }
    ...
    private BaseMsg createShookHands(byte length, byte[] info) {
        ...
        if (length<=7) {
            ...
            int deviceId = ByteUtils.getInt(new byte[]{info[0], info[1],info[2], info[3]});
            short end = ByteUtils.getShort(new byte[]{info[4], info[5]});
            shookHandsMsg = new ShookHandsMsg(SHOOKHANDS, length, (byte) 0, (byte) 0, end, deviceId);
        }
    ...
    private BaseMsg createBootloadShookInit(byte length, byte[] info) {
        ...
        if (length <= 7) {
            ...
            byte boardId = info[1];
            short end = ByteUtils.getShort(new byte[]{info[2], info[3]});
            bootloadShookhandInit =  new BootloadShookhandInit(BOOTLOADINIT, length, stationId, boardId, end);
        }else {
            int deviceId = ByteUtils.getInt(new byte[]{info[0], info[1],info[2], info[3]});
            short end = ByteUtils.getShort(new byte[]{info[4], info[5]});
            bootloadShookhandInit = new BootloadShookhandInit(BOOTLOADINIT, length, (byte) 0, (byte) 0, end, deviceId);
        }
