...
public class LongLinkHandler extends ChannelInboundHandlerAdapter {
    ...
    @Autowired
    private FileThreadPoolComponent fileThreadPoolComponent;
    @Autowired
    private BootloaderTaskManage bootloaderTaskManage;
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        switch (baseMsg.getType()) {
            ...
            case BOOTLOADER_INIT:
                ...
                LOGGER.info("forward update request from："+ mecheineId);
                String path = clientBinFileManage.removeBinFile(mecheineId);
                if (!bootloaderTaskManage.hasTask(mecheineId)){
                    //任务没有生成
                    LOGGER.info("任务没有生成，添加");
                    bootloaderTaskManage.addTask(mecheineId,new BootloadTask(mecheineId));
                    bootloaderTaskManage.setBootloaderRequest(mecheineId,
                            new BootloaderRequest(mecheineId,path,TimeUtils.getCurrentTimeStamp10(),true));
                }
            ...
            case BOOTLOADSTATUS:
                ...
                BootloaderStatusEnum statusEnum = bootloadStatus.getStatus() == (byte)1 ?
                                                            BootloaderStatusEnum.SUCCESS : BootloaderStatusEnum.FAILED;
